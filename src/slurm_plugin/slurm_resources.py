...
class SlurmPartition:
    ...
    def get_online_node_by_type(self, terminate_drain_nodes, terminate_down_nodes,
                                ec2_missing_backing_instance_limit):
...
class SlurmNode(metaclass=ABCMeta):
    ...
    @abstractmethod
    def is_healthy(self, terminate_drain_nodes, terminate_down_nodes, ec2_missing_backing_instance_limit,
    ...
    def is_backing_instance_valid(self, ec2_missing_backing_instance_limit, check_type, log_warn_if_unhealthy=True):
        ...
        if self.is_nodeaddr_set():
            if not self.instance:
                ...
                if ec2_missing_backing_instance_limit != -1:
                    node_failure_count[check_type].pop(self.name, None)
        return True
...
class StaticNode(SlurmNode):
    ...
    def is_healthy(self, terminate_drain_nodes, terminate_down_nodes, ec2_missing_backing_instance_limit, log_warn_if_unhealthy=True):
        """Check if a slurm node is considered healthy."""
        return (
